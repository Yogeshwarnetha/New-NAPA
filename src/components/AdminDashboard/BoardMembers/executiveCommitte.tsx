...
import { fetchExecutiveCommitteePagination, updateExecutiveCommittee, deleteExecutiveCommittee } from "../../../apirequest/boardMember";
import CreateExecutiveCommittee from "./createExecutiveCommittee";
...
const ExecutiveCommitteeDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetchExecutiveCommitteePagination(page, limit);
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateExecutiveCommittee(editMember.id, formData);
      handleEditClose();
      setPage(1);
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteExecutiveCommittee(id);
      setPage(1);
    } catch (error) {
