...
import { fetchChapterDirectorsPagination } from "../../../apirequest/boardMember";
import CreateChapterLeads from "./createchapterdirectors";
import AdminDashboardLayout from "..";

interface ChapterLeads {
    id: number;
    name: string;
    imageUrl: string;
}

const CreateChapterDirectorsDashboard = () => {
    const [chapterLeads, setChapterLeads] = useState<ChapterLeads[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await fetchChapterDirectorsPagination(page, limit);
                setChapterLeads(response.data || []);
                                ...
                                <TableBody>
                                    {chapterLeads.map((member) => (
                                        <TableRow key={member.id}>
                                            <TableCell sx={{ fontSize: 14 }}>{chapterLeads.indexOf(member) + 1 + (page - 1) * limit}</TableCell>
...
export default CreateChapterDirectorsDashboard;
